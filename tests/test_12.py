...
def test_calc_to_gen_20_sample():
    ...
    assert_equal(offset, 7, f"not correct offset calculated{next_gen}")
...
def test_calc_sum_50billion_gen_real():
    # I has found out that after 200 generation the number increases with 96 for each generation
    with open(input_file_path) as f:
        my_input = f.read().strip()
    state, offset = p12.calc_to_generations(my_input, 200)
    my_sum = p12.calc_sum(state, offset)
    my_sum = my_sum + (50000000000-200)*96
